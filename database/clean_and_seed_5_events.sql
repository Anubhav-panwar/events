-- ==============================================================================
-- CLEAN DATABASE (EXCEPT USERS) & SEED 5 UNIQUE CURATED EVENTS
-- Target: MySQL 5.7+ / 8.0+ / MariaDB 10.x / cPanel phpMyAdmin
-- Safely truncates previous events, tickets, orders, and media while PRESERVING:
--   - `users` table (admin, vendor, customer logins remain 100% untouched)
--   - `migrations` table
--   - `vendor_profiles` (slugs validated and guaranteed non-null)
-- ==============================================================================

SET FOREIGN_KEY_CHECKS = 0;

-- ------------------------------------------------------------------------------
-- STEP 1: Truncate / Clear all event, order, and transactional tables
-- ------------------------------------------------------------------------------
TRUNCATE TABLE `tickets`;
TRUNCATE TABLE `order_items`;
TRUNCATE TABLE `orders`;
TRUNCATE TABLE `event_saves`;
TRUNCATE TABLE `event_reminders`;
TRUNCATE TABLE `event_referrals`;
TRUNCATE TABLE `event_share_clicks`;
TRUNCATE TABLE `event_media`;
TRUNCATE TABLE `ticket_types`;
TRUNCATE TABLE `events`;
TRUNCATE TABLE `posts`;
TRUNCATE TABLE `vendor_follows`;
TRUNCATE TABLE `vendor_media`;
TRUNCATE TABLE `category_vendor_profile`;

-- ------------------------------------------------------------------------------
-- STEP 2: Ensure Vendor Profiles Have Valid Slugs & Default Profile Exists
-- ------------------------------------------------------------------------------
-- Fix any missing or empty slugs in existing vendor profiles
UPDATE `vendor_profiles` 
SET `slug` = CONCAT('vendor-', `id`) 
WHERE `slug` IS NULL OR `slug` = '';

-- Ensure primary vendor profile (ID 1) exists for events
INSERT INTO `vendor_profiles` (
    `id`, `user_id`, `business_name`, `slug`, `description`,
    `phone`, `website`, `instagram`, `facebook`, `twitter`,
    `address`, `city`, `country`, `latitude`, `longitude`,
    `opening_hours`, `is_approved`, `created_at`, `updated_at`
) VALUES (
    1,
    COALESCE((SELECT `id` FROM `users` WHERE `role` = 'vendor' ORDER BY `id` ASC LIMIT 1), (SELECT `id` FROM `users` ORDER BY `id` ASC LIMIT 1)),
    'Acme Events', 'acme-events', 'Premier event management, live concerts, esports, and cultural festival productions worldwide.',
    '+1-555-0199', 'https://example.com', 'https://instagram.com/acmeevents', 'https://facebook.com/acmeevents', 'https://twitter.com/acmeevents',
    '8440 Sunset Boulevard', 'Los Angeles', 'United States', 34.092809, -118.371300,
    NULL, 1, NOW(), NOW()
)
ON DUPLICATE KEY UPDATE
    `slug` = IF(`slug` IS NULL OR `slug` = '', 'acme-events', `slug`),
    `is_approved` = 1,
    `updated_at` = NOW();

-- ------------------------------------------------------------------------------
-- STEP 3: Ensure Standard Event Categories Exist (IDs 1, 2, 3, 4)
-- ------------------------------------------------------------------------------
INSERT INTO `categories` (`id`, `name`, `slug`, `created_at`, `updated_at`) VALUES
(1, 'Music & Concerts', 'music', NOW(), NOW()),
(2, 'Sports & Esports', 'sports', NOW(), NOW()),
(3, 'Tech & Workshops', 'workshops', NOW(), NOW()),
(4, 'Food & Culinary', 'food-drink', NOW(), NOW())
ON DUPLICATE KEY UPDATE
    `name` = VALUES(`name`),
    `slug` = VALUES(`slug`),
    `updated_at` = NOW();
...
-- ------------------------------------------------------------------------------
-- STEP 4: Insert 5 Unique Curated Events (IDs 25 - 29)
-- ------------------------------------------------------------------------------
INSERT INTO `events` (
...
(
    ...
    'published', 1, 'paid', 45.00, 'solaris-sunset-rooftop-sessions',
...
(
    ...
    'published', 1, 'paid', 35.00, 'apex-esports-championship-grand-finals',
...
(
    ...
    'published', 1, 'paid', 150.00, 'mastering-next-gen-robotics-and-autonomous-drones',
...
(
    ...
    'published', 1, 'free', 0.00, 'world-street-food-and-night-market-carnival',
...
(
    ...
    'published', 1, 'paid', 30.00, 'neon-cyberpunk-indie-film-and-synthwave-night',
...
-- ------------------------------------------------------------------------------
-- STEP 5: Insert Ticket Types for Events 25 - 29
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- STEP 6: Insert Media Rows for Events 25 - 29 (event_media table)
-- ------------------------------------------------------------------------------
